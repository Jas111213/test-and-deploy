  ...
  useEffect(() => {
    ...
    const today = sortedTasks.filter((task) => {
      if (
        ...
        task.deadline < new Date().setDate(new Date().getDate() + 1)
      ) {
        return true;
      }
      return false;
    ...
    const thisWeek = sortedTasks.filter((task) => {
      if (
        task.deadline &&
        task.deadline > new Date().getTime() &&
        task.deadline < new Date().setDate(new Date().getDate() + 7)
      ) {
        return true;
      }
      return false;
  ...
  return (
    <>
      <div className="todo-list-header">
        <form onSubmit={createTask} className="task-input-container">
          {" "}
          <input
      ...
      <div className="tasks-container">
        {/* Today Tasks */}
        {categorisedTasks.today.filter((task) => !task.completed).length >
          0 && (
          <div className="tasks-container-header">
            <h3>Today</h3>
            {categorisedTasks.today
        ...
        {/* This Week Tasks */}
        {categorisedTasks.thisWeek
          .filter((task) => !task.completed)
          .filter((task) => !categorisedTasks.today.includes(task)).length >
          0 && (
          <div className="tasks-container-header">
            <h3>This Week</h3>
            {/* filter today tasks */}
            {categorisedTasks.thisWeek
              .filter((task) => !categorisedTasks.today.includes(task))
              .map((task, index) => (
                <TaskItem task={task} index={index} key={index} />
              ))}
          </div>
        ...
        {/* Overdue Tasks */}
        {categorisedTasks.overDue.length > 0 && (
          <div className="tasks-container-header">
            <h3>Overdue</h3>
            {categorisedTasks.overDue.map((task, index) => (
              <TaskItem task={task} index={index} key={index} overDue={true} />
            ))}
          </div>
        )}
        ...
        {sortedTasks.length > 0 && (
          <div className="tasks-container-header">
            {sortedTasks
              .filter((task) => !task.completed)
              .filter((task) => !categorisedTasks.today.includes(task))
              .filter((task) => !categorisedTasks.thisWeek.includes(task))
              .filter((task) => !categorisedTasks.overDue.includes(task))
              .map((task, index) => (
                <TaskItem task={task} index={index} key={index} />
              ))}{" "}
          </div>
        ...
        {/* Completed Tasks */}
        {categorisedTasks.completed.length > 0 && (
          <div className="tasks-container-header">
            <h3>Completed</h3>
            {categorisedTasks.completed.map((task, index) => (
              <TaskItem task={task} index={index} key={index} />
            ))}
          </div>
        )}
